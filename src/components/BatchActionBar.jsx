...
import { formatMagnetUri, copyToClipboard } from '../utils/magnetHelper';
...
export default function BatchActionBar({
  ...
  const handleCopyAllMagnets = () => {
    const magnets = selectedItems
      .map(item => formatMagnetUri(item))
      .filter(Boolean)
      .join('\n\n');

    if (magnets) {
      copyToClipboard(magnets);
      setCopiedAll(true);
  ...
  const handleExportTxt = () => {
    const content = selectedItems.map((item, idx) => {
      const mag = formatMagnetUri(item);
  ...
  const handleDownloadAllZip = async () => {
    ...
    try {
      ...
      selectedItems.forEach((item, index) => {
        ...
        const mag = formatMagnetUri(item);
        if (mag) {
          zip.file(`${safeName}.magnet`, mag);
        }
      });

      // Add summary list
      const summary = selectedItems.map((it, i) => `${i + 1}. ${it.title} (${it.size || ''})\n   ${formatMagnetUri(it) || it.torrent || ''}`).join('\n\n');
