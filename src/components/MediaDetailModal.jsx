...
export default function MediaDetailModal({
  ...
  const handleCopyTitle = () => {
    copyToClipboard(details?.title || item.title);
    setCopiedTitle(true);
    setTimeout(() => setCopiedTitle(false), 2000);
  };

  const handleOpenMagnet = () => {
    openMagnetInClient({ title: details?.title || item.title, magnet: magnetUri, torrent: torrentUrl });
  ...
  const handleDownloadTorrent = () => {
    downloadTorrentFile({ title: details?.title || item.title, torrent: torrentUrl, magnet: magnetUri });
  };
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                      {magnetUri ? (
                        <button
                          ...
                          className="px-4 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-md hover:shadow-purple-500/20 transition cursor-pointer"
                        >
                          <Magnet className="w-4 h-4" />
                          {copiedMagnet ? 'Magnet Dispatched!' : 'Open Magnet Link'}
                        </button>
                      ) : (
                        <button
                          disabled
