...
import { openMagnetInClient, downloadTorrentFile, downloadMagnetFile, copyToClipboard, formatMagnetUri, getValidPosterUrl, defaultPoster } from '../utils/magnetHelper';
...
export default function TorrentDetailsPage({
  ...
  const posterUrl = getValidPosterUrl(details?.poster || item?.image);
  ...
  const handleOpenMagnet = () => {
    openMagnetInClient({ title: rawTitle, magnet: magnetUri, torrent: torrentUrl });
    setCopiedMagnet(true);
    setTimeout(() => setCopiedMagnet(false), 2200);
  };
  ...
  const handleDownloadMagnetFile = () => {
    downloadMagnetFile({ title: rawTitle, magnet: magnetUri });
  };

  const handleCopyNfo = () => {
    if (details?.metadata) {
      copyToClipboard(details.metadata);
      setCopiedNfo(true);
  ...
  const handleDownloadTorrent = () => {
    downloadTorrentFile({ title: rawTitle, torrent: torrentUrl, magnet: magnetUri });
  };
              ...
              <div className="p-4 space-y-2.5">
                <div className="grid grid-cols-2 gap-2.5">
                  ...
                  {magnetUri ? (
                    <button
                      ...
                      className="w-full py-3 px-3 rounded-xl bg-rose-600 hover:bg-rose-500 active:bg-rose-700 text-white font-bold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg shadow-rose-950/50 transition transform active:scale-95 cursor-pointer"
                      title="Open Magnet URI in BitTorrent client & copy to clipboard"
                    >
                      <Magnet className="w-4 h-4" />
                      <span>{copiedMagnet ? 'Dispatched!' : 'magnet'}</span>
                    </button>
                  ) : (
                    <button
                      disabled
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <button
                    ...
                    disabled={!magnetUri}
                    className="py-2 px-2.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 rounded-xl text-xs font-medium text-slate-300 flex items-center justify-center gap-1.5 transition shadow-sm"
                    title="Copy full magnet link with trackers"
                  >
                    {copiedMagnet ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-purple-400" />}
                    <span>{copiedMagnet ? 'Copied' : 'Copy Magnet'}</span>
                  </button>
                  ...
                  <button
                    ...
                    disabled={!magnetUri}
                    className="py-2 px-2.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 rounded-xl text-xs font-medium text-slate-300 flex items-center justify-center gap-1.5 transition shadow-sm"
                    title="Download as .magnet text file"
                  >
